...
class FilteredNoise(nn.Module):
    ...
    def forward(self, z):
        ...
        # Add batch dimension if not existing
        if len(z['H'].shape) == 2:
            INPUT_FILTER_COEFFICIENT.unsqueeze(0)
        ...
        zero_phase_ir_bank = torch.fft.irfft(torch.view_as_complex(ZERO_PHASE_FR_BANK), n = (filter_coeff_length * 2 - 1)) #torch.irfft(ZERO_PHASE_FR_BANK, 1, signal_sizes = (filter_coeff_length * 2 - 1,))
        ...
        ZERO_PADED_WINDOWED_LINEAR_PHASE_FR_BANK = torch.view_as_real(torch.fft.rfft(zero_paded_windowed_linear_phase_ir_bank)) #torch.rfft(zero_paded_windowed_linear_phase_ir_bank, 1)
        ...
        ZERO_PADED_NOISE = torch.view_as_real(torch.fft.rfft(zero_paded_noise)) #torch.rfft(zero_paded_noise, 1)
        ...
        filtered_noise = torch.fft.irfft(torch.view_as_complex(FILTERED_NOISE)).view(batch_num, frame_num, -1) * self.attenuate_gain #torch.irfft(FILTERED_NOISE, 1).view(batch_num, frame_num, -1)          
