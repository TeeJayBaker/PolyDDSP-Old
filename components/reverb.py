...
class TrainableFIRReverb(nn.Module):
    ...
    def forward(self, z):
        ...
        INPUT_SIGNAL = torch.view_as_real(torch.fft.rfft(zero_pad_input_signal)) #torch.rfft(zero_pad_input_signal, 1)
        ...
        FIR = torch.view_as_real(torch.fft.rfft(zero_pad_final_fir)) #torch.rfft(zero_pad_final_fir, 1)
        ...
        output_signal = torch.fft.irfft(torch.view_as_complex(OUTPUT_SIGNAL)) #torch.irfft(OUTPUT_SIGNAL, 1)
