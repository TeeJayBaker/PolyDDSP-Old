...
class Trainer:
    ...
    def train(
        ...
        for i in range(start, start + num_unit, validation_interval):
            ...
            print_row(kwarg_list=[""] * len(kwarg_list), pad="-")
            self.dataframe = pd.concat([self.dataframe, # self.dataframe = self.dataframe.append(
                pd.DataFrame([dict(zip(kwarg_list, print_kwarg))])], ignore_index=True
            )
    ...
    def update_experiment(self):
        ...
        if os.path.exists(Trainer.experiment_name + ".csv"):
            ...
            df_ex = pd.concat([df_ex, df_config], sort=False)# df_ex = df_ex.append(df_config, sort=False)
        else:
            df_ex = df_config
